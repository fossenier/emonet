...
parser.add_argument(
    "--image_path", type=str, default="images/example.png", help="Path to a face image."
)
...
with torch.no_grad():
    ...
    predicted_emotion_class = (
        torch.argmax(nn.functional.softmax(output["expression"], dim=1)).cpu().item()
    )

    # Expected output on example image: Predicted Emotion Happy - valence 0.064 - arousal 0.143
    print(
        f"Predicted Emotion {emotion_classes[predicted_emotion_class]} - valence {output['valence'].clamp(-1.0,1.0).cpu().item():.3f} - arousal {output['arousal'].clamp(-1.0,1.0).cpu().item():.3f}"
    )
